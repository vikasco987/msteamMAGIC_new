      ...
      {paymentHistory.slice().reverse().map((entry, i) => {
        ...
        return (
          ...
          >
            ...
            {entry.fileUrl || entry.utr ? (
              <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 pt-2 border-t border-gray-50">
                {entry.utr && (
                  <div>
                    <p className="text-[9px] uppercase tracking-wider text-gray-400">UTR No.</p>
                    <p className="font-bold text-gray-800 select-all">{entry.utr}</p>
                  </div>
                )}
                {entry.fileUrl && (
                  <a
                    ...
                    className="inline-flex items-center gap-1 text-blue-600 hover:text-blue-800 font-medium transition-colors mt-auto mb-0.5"
                  >
