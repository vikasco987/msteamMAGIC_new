...
export async function POST(
  ...
  if (action === "create-link") {
    try {
      ...
      if (!appId || !secretKey) {
        return NextResponse.json({ success: false, message: "Razorpay API keys missing" }, { status: 500 });
      }
      ...
      await prisma.razorpayLink.create({
        data: {
          orderId,
          name: name || "Customer",
          email: email || "",
      ...
      console.error("❌ RAZORPAY API REJECTED REQUEST:", cfError);
      
      return NextResponse.json({ 
        success: false, 
        message: typeof cfError === 'object' ? (cfError.message || "Razorpay API Error") : cfError
      }, { status: 500 });
    }
  }
...
export async function GET(
  ...
  if (action === "get-all-links") {
    try {
      ...
      const totalCount = await prisma.razorpayLink.count({ where });
      const totalPages = Math.ceil(totalCount / limit);

      const links = await prisma.razorpayLink.findMany({
        where,
  ...
  if (action === "get-reports") {
    try {
      ...
      const allLinks = await prisma.razorpayLink.findMany({
        where,
        orderBy: { createdAt: "desc" }
      });
  ...
  if (action === "user-details") {
    ...
    try {
      // Find from local RazorpayLink history first
      // @ts-ignore
      const lastLink = await prisma.razorpayLink.findFirst({
        where: {
          OR: [
            { email: identifier },
            { phone: identifier }
  ...
  if (action === "check-status") {
    ...
    try {
      // Proxy status check to magicscale.in backend to handle actual status tracking (whether Razorpay or Razorpay)
      const backendUrl = `https://magicscale.in/api/razorpay/check-status?order_id=${orderId}`;
      ...
      // Update local DB
      await prisma.razorpayLink.update({
        where: { orderId },
