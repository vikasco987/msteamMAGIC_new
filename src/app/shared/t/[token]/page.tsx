...
import { 
  ...
  Clock, CheckSquare, AlertCircle, Share2, Timer
} from "lucide-react";
...
const CountdownTimer = ({ expiresAt }: { expiresAt: number }) => {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      ...
      if (remaining === 0) {
        ...
        // Automatically reload to show expired state if it just expired
  ...
  const h = Math.floor(timeLeft / (3600 * 1000));
  const m = Math.floor((timeLeft % (3600 * 1000)) / (60 * 1000));
...
export default function SharedTokenTaskPage() {
  ...
  useEffect(() => {
    const fetchTask = async () => {
      try {
        const res = await fetch(`/api/public-tasks/verify?token=${token}`);
  ...
  if (errorMsg || !task) {
    return (
      <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center justify-center p-6">
        ...
        <h1 className="text-2xl font-black text-slate-900">{errorMsg === "Link Expired" ? "Link Expired" : "Task Not Found"}</h1>
        <p className="text-slate-400 font-bold mt-2 text-center max-w-sm">
          {errorMsg === "Link Expired" 
            ? "This share link was valid for 24 hours and has now expired. Please ask the administrator to generate a new link." 
            : "This share link is either incorrect or has been revoked by the administrator."}
        </p>
        <button
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] py-12 px-4 md:px-8">
      ...
      {/* Security & Access Badges with Expiration Timer */}
      <div className="max-w-2xl mx-auto mb-8 flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="flex gap-3">
          ...
          {expiresAt && (
            <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-orange-100 shadow-sm text-[9px] font-black text-slate-400 uppercase tracking-widest">
          ...
          <div className="grid grid-cols-1 gap-y-2">
            {cf.shopName && (
              <div className="flex items-center justify-between p-3 bg-slate-50/60 rounded-xl hover:bg-slate-50 transition-all">
                <span className="text-xs font-bold text-slate-500 flex items-center gap-2">🏪 Shop Name</span>
                <span className="text-xs font-black text-slate-800 flex items-center gap-1.5">
                  {cf.shopName}
                  <button onClick={() => handleCopyText(cf.shopName, "Shop name")} className="text-slate-400 hover:text-indigo-600"><Copy size={12} /></button>
                </span>
              </div>
            )}
            ...
            {cf.outletName && (
              <div className="flex items-center justify-between p-3 bg-slate-50/60 rounded-xl hover:bg-slate-50 transition-all">
                <span className="text-xs font-bold text-slate-500 flex items-center gap-2">🏷️ Outlet Name</span>
                <span className="text-xs font-black text-slate-800 flex items-center gap-1.5">
                  {cf.outletName}
                  <button onClick={() => handleCopyText(cf.outletName, "Outlet name")} className="text-slate-400 hover:text-indigo-600"><Copy size={12} /></button>
                </span>
              </div>
            )}
            ...
            {cf.customerName && (
              <div className="flex items-center justify-between p-3 bg-slate-50/60 rounded-xl hover:bg-slate-50 transition-all">
                <span className="text-xs font-bold text-slate-500 flex items-center gap-2">👤 Customer</span>
                <span className="text-xs font-black text-slate-800 flex items-center gap-1.5">
                  {cf.customerName}
                  <button onClick={() => handleCopyText(cf.customerName, "Customer name")} className="text-slate-400 hover:text-indigo-600"><Copy size={12} /></button>
                </span>
              </div>
            )}
            ...
            {cf.phone && (
              <div className="flex items-center justify-between p-3 bg-slate-50/60 rounded-xl hover:bg-slate-50 transition-all">
                <span className="text-xs font-bold text-slate-500 flex items-center gap-2">📞 Phone Number</span>
                <a href={`tel:${cf.phone}`} className="text-xs font-black text-indigo-600 hover:underline flex items-center gap-1.5">
                  {cf.phone}
                  <button type="button" onClick={(e) => { e.preventDefault(); handleCopyText(cf.phone, "Phone"); }} className="text-slate-400 hover:text-indigo-600"><Copy size={12} /></button>
                </a>
              </div>
            )}
            ...
            {cf.email && (
              <div className="flex items-center justify-between p-3 bg-slate-50/60 rounded-xl hover:bg-slate-50 transition-all">
                <span className="text-xs font-bold text-slate-500 flex items-center gap-2">📧 Email</span>
                <a href={`mailto:${cf.email}`} className="text-xs font-black text-indigo-600 hover:underline flex items-center gap-1.5">
                  {cf.email}
                  <button type="button" onClick={(e) => { e.preventDefault(); handleCopyText(cf.email, "Email"); }} className="text-slate-400 hover:text-indigo-600"><Copy size={12} /></button>
                </a>
              </div>
            )}
            ...
            {gstin && (
              <div className="flex items-center justify-between p-3 bg-slate-50/60 rounded-xl hover:bg-slate-50 transition-all">
                <span className="text-xs font-bold text-slate-500 flex items-center gap-2">📜 GSTIN / Tax No.</span>
                <span className="text-xs font-black font-mono text-slate-800 flex items-center gap-1.5">
                  {gstin}
                  <button onClick={() => handleCopyText(String(gstin), "GSTIN")} className="text-slate-400 hover:text-indigo-600"><Copy size={12} /></button>
                </span>
              </div>
            )}
            ...
            {cf.packageAmount && (
              <div className="flex items-center justify-between p-3 bg-slate-50/60 rounded-xl hover:bg-slate-50 transition-all">
                <span className="text-xs font-bold text-slate-500 flex items-center gap-2">💰 Package Amount</span>
                <span className="text-xs font-black text-slate-800">₹{cf.packageAmount}</span>
              </div>
            )}
          </div>
                ...
                {accountHolder && (
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-black text-slate-800 flex items-center gap-1">
                      {accountHolder}
                      <button onClick={() => handleCopyText(String(accountHolder), "Account Holder")} className="text-slate-400 hover:text-indigo-600"><Copy size={12} /></button>
                    </span>
                  </div>
                )}
                {bankName && (
                ...
                {accountNumber && (
                  <div className="flex items-center justify-between text-xs bg-white/70 p-2 rounded-xl border border-indigo-100">
                    ...
                    <span className="font-black font-mono text-indigo-700 flex items-center gap-1 text-sm">
                      {accountNumber}
                      <button onClick={() => handleCopyText(String(accountNumber), "Account Number")} className="text-slate-400 hover:text-indigo-600"><Copy size={12} /></button>
                    </span>
                  </div>
                )}
                {ifscCode && (
                  <div className="flex items-center justify-between text-xs bg-white/70 p-2 rounded-xl border border-indigo-100">
                    ...
                    <span className="font-black font-mono text-indigo-700 flex items-center gap-1 text-sm">
                      {ifscCode}
                      <button onClick={() => handleCopyText(String(ifscCode), "IFSC Code")} className="text-slate-400 hover:text-indigo-600"><Copy size={12} /></button>
                    </span>
                  </div>
                )}
                {bankBranch && (
                ...
                {upiId && (
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-black text-slate-800 font-mono flex items-center gap-1">
                      {upiId}
                      <button onClick={() => handleCopyText(String(upiId), "UPI ID")} className="text-slate-400 hover:text-indigo-600"><Copy size={12} /></button>
                    </span>
                  </div>
                )}
              </div>
