...
import { Users, Search, Lock, Unlock, ShieldCheck, XCircle, CheckCircle2, FileEdit, Banknote, Eye, DollarSign, Archive, Filter } from "lucide-react";
...
export default function EmployeeDirectoryPage() {
    ...
    const handleEditClick = (emp: any) => {
        ...
        setEditForm({
            ...
            lastWorkingDate: emp.lastWorkingDate ? emp.lastWorkingDate.substring(0, 10) : "",
            exitReason: emp.exitReason || "",
            finalSettlementStatus: emp.finalSettlementStatus || "Pending"
        });
    };

    const handleArchiveEmployee = async (emp: any) => {
        if (!confirm(`Are you sure you want to archive ${emp.name}? This will mark them as Inactive and stop their payroll/attendance.`)) return;
        const toastId = toast.loading("Archiving employee...");
        try {
            const res = await fetch(`/api/admin/employees/${encodeURIComponent(emp.email)}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ employmentStatus: "Inactive", lastWorkingDate: new Date().toISOString() })
            });
            if (res.ok) {
                toast.success("Employee archived successfully", { id: toastId });
                fetchEmployees();
            } else {
                toast.error("Failed to archive employee", { id: toastId });
            }
        } catch (error) {
            toast.error("Network error while archiving", { id: toastId });
        }
    };
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="space-y-5">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs font-black text-slate-400 uppercase mb-1">Employment Status</label>
                                        <select 
                                    ...
                                    <div>
                                        <label className="block text-xs font-black text-slate-400 uppercase mb-1">Last Working Date</label>
                                        <input type="date" value={editForm.lastWorkingDate} onChange={e => setEditForm({...editForm, lastWorkingDate: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:border-indigo-500 outline-none" />
                                    </div>
                                </div>
                                {editForm.employmentStatus === 'Inactive' && (
                                    <div className="grid grid-cols-2 gap-4 bg-rose-50 p-4 rounded-xl border border-rose-100">
                                        <div className="col-span-2">
                                            <label className="block text-xs font-black text-rose-500 uppercase mb-1">Exit Reason</label>
                                            <input type="text" placeholder="e.g. Resigned, Terminated" value={editForm.exitReason} onChange={e => setEditForm({...editForm, exitReason: e.target.value})} className="w-full bg-white border border-rose-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:border-rose-500 outline-none" />
                                        </div>
                                        <div className="col-span-2">
                                            <label className="block text-xs font-black text-rose-500 uppercase mb-1">Final Settlement Status</label>
                                            <select 
                                                value={editForm.finalSettlementStatus} 
                                                onChange={e => setEditForm({...editForm, finalSettlementStatus: e.target.value})}
                                                className="w-full py-3 px-4 bg-white border border-rose-200 rounded-xl text-sm font-bold text-slate-900 focus:border-rose-500 outline-none"
                                            >
                ...
                <div className="flex gap-3 w-full md:w-auto">
                    <div className="w-full md:w-48 relative">
                        ...
                        <select 
                            ...
                            className="w-full pl-10 pr-4 py-4 bg-white border-2 border-slate-100 rounded-2xl text-sm font-bold focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all shadow-sm appearance-none"
                        >
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex flex-col gap-2">
                                            ...
                                            {(emp.employmentStatus || 'Active') === 'Active' && (
                                                <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-emerald-600 bg-emerald-100 px-2.5 py-1 rounded-md w-fit mt-1">
                                                    🟢 Active
                                                </span>
                                            )}
                                            {emp.employmentStatus === 'Notice Period' && (
                                                <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-amber-600 bg-amber-100 px-2.5 py-1 rounded-md w-fit mt-1">
                                                    🟡 Notice Period
                                                </span>
                                            )}
                                            {emp.employmentStatus === 'Inactive' && (
                                                <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-rose-600 bg-rose-100 px-2.5 py-1 rounded-md w-fit mt-1">
                                                    🔴 Inactive
                                                </span>
                                            )}
                                            {emp.employmentStatus === 'Suspended' && (
                                                <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-slate-600 bg-slate-100 px-2.5 py-1 rounded-md w-fit mt-1 border border-slate-300">
                                                    ⚪️ Suspended
                                                </span>
                                            )}
                                        </div>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            ...
                                            <Link 
                                                ...
                                                className="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-xl transition-all inline-block"
