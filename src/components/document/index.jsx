...
export default function MagicScaleAgreementPDF({ company, client, agreement, payment, customHtml }) {
  if (!company || !client || !agreement) return null;

  const title = `${client.name?.trim().replace(/\s+/g, '_') || 'Client'}_Consultant_Agreement_${(agreement.date || '').toString().replace(/\s+/g, '_')}`;
